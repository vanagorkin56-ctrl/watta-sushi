...
- Delivery quotes use fixed Google endpoints, validate an exact Amsterdam locality, bind the route distance to a normalized address hash and expire after 15 minutes.
...
- Store Stripe, Google Maps and SMTP credentials in the host's secret manager. Restrict the Google key to the required APIs and deployment source addresses.
- Use one persistent instance with SQLite. For multiple replicas or serverless hosting, move orders, quotes and rate limits to shared services such as PostgreSQL and Redis.
