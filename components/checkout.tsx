...
export function Checkout(){
 ...
 function changeAddress(setter:(s:string)=>void,value:string){setter(value);setQuote(null);addressVersion.current++;}
 async function calculate(){setQuoting(true);setError('');const version=addressVersion.current;try{const r=await fetch('/api/delivery-quote',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({street,postcode,city})});const data=await r.json();if(!r.ok)throw new Error(data.error);if(version===addressVersion.current)setQuote(data);}catch(e){if(version===addressVersion.current)setError(e instanceof Error?e.message:'deliveryUnavailable');}finally{setQuoting(false);}}
 async function submit(event:FormEvent<HTMLFormElement>){
  ...
  const f=new FormData(event.currentTarget);const payload={locale,...(fulfillment==='delivery'&&quote?{quoteId:quote.quoteId}:{}),items:cart,customer:{name:String(f.get('name')),email:String(f.get('email')),phone:String(f.get('phone')),fulfillment,street,postcode,city,comment:String(f.get('comment')||'')}};
  ...
  try{const response=await fetch('/api/checkout',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({...payload,requestId:lastRequest.current!.id})});const data=await response.json();if(!response.ok){if(['requestConflict','sessionExpired'].includes(data.error))lastRequest.current=null;throw new Error(data.error);}if(!data.url)throw new Error('genericError');window.location.assign(data.url);}catch(e){setError(e instanceof Error?e.message:'genericError');setBusy(false);}
 }
 ...
 return <div className="page-wrap container">
  ...
  <div className="checkout-grid"><form className="form-panel" onSubmit={submit}>
   ...
   {fulfillment==='delivery'?<>
    ...
    {quote&&<p className="notice">{t.distance}: {(quote.distanceMeters/1000).toFixed(2)} km · {money(quote.deliveryFee,locale)}<br/><small>Google Maps</small></p>}
   ...
   <label className="form-terms"><input type="checkbox" required/><span>{t.policy} <Link href="/delivery">{t.delivery}</Link> · <Link href="/privacy">{t.privacy}</Link></span></label>
   {!config.checkoutReady&&<p className="notice">{t.unavailable} <Link href="/contacts">{t.contactUs}</Link></p>}{error&&<p className="notice" role="alert">{message(error)}</p>}
