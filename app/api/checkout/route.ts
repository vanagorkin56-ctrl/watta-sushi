...
export async function POST(request:Request){
 try{
  ...
  if(Number(request.headers.get('content-length'))>20000)return NextResponse.json({error:'invalidInput'},{status:413});
  const raw=await request.text();if(raw.length>20000)return NextResponse.json({error:'invalidInput'},{status:413});
  ...
  if(order.session_id){
   ...
   return NextResponse.json({url:session.url||`${origin}/success?session_id=${session.id}`});
  }
 ...
 }catch(e){if(e instanceof OrderError)return NextResponse.json({error:e.code},{status:400});console.error('Checkout failed:',e instanceof Error?e.name:'Unknown');return NextResponse.json({error:'genericError'},{status:500});}
}
