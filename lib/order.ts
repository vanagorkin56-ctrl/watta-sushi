...
export const checkoutSchema = z.object({
 acceptedTerms:z.literal(true), requestId: z.string().uuid(), quoteId: z.string().min(1).max(1024).optional(), locale: z.enum(['nl','en','uk']),
...
export function priceOrder(input: CheckoutInput, config: ShopConfig, distanceMeters?: number) {
 ...
 if(c.fulfillment==='delivery' && (typeof distanceMeters!=='number'||!Number.isFinite(distanceMeters)||distanceMeters<0||distanceMeters>1_000_000)) throw new OrderError('quoteRequired');
