...
export async function deliveryQuote(address:z.infer<typeof addressSchema>){
 ...
 const routeResponse=await fetch('https://routes.googleapis.com/directions/v2:computeRoutes',{method:'POST',headers:{'Content-Type':'application/json','X-Goog-Api-Key':key,'X-Goog-FieldMask':'routes.distanceMeters'},body:JSON.stringify({origin:{address:shopConfig().pickupAddress},destination:{location:{latLng:{latitude:destination.lat,longitude:destination.lng}}},travelMode:'DRIVE',routingPreference:'TRAFFIC_UNAWARE',computeAlternativeRoutes:false}),signal:AbortSignal.timeout(10000),cache:'no-store'});
 ...
 return {distanceMeters:meters,deliveryFee:deliveryAmount(meters),formattedAddress:result.formatted_address};
}
