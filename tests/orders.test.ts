...
test('rejects client prices, noninteger amounts, unavailable products and aggregate quantity attacks',()=>{const i=input();assert.equal(checkoutSchema.safeParse({...i,items:[{productId:21,quantity:2,price:1}]}).success,false);assert.equal(checkoutSchema.safeParse({...i,items:[{productId:21,quantity:1.1}]}).success,false);assert.throws(()=>priceOrder({...i,items:[{productId:999999,quantity:2}]},shopConfig()),/invalidItems/);assert.throws(()=>priceOrder({...i,items:[{productId:21,quantity:30},{productId:21,quantity:1}]},shopConfig()),/invalidItems/);});
